...
func processTemplateResources(
	...
	// Prepare to track desired resources
	desiredResources := make(map[string]*unstructured.Unstructured)
	desiredResourceIdentifiers := make([]ResourceIdentifier, 0)
	...
	for i := 0; i < o.Output.ResourceCount(); i++ {
		...
		// Build resource key
		key := resourceKey(unstructuredObj)

		// Add to desired resources
		desiredResources[key] = unstructuredObj
		...
		desiredResourceIdentifiers = append(desiredResourceIdentifiers, ri)
	}
	...
	existingResources := make(map[string]ResourceIdentifier)
	for _, ri := range existingResourceIdentifiers {
	...
	for _, ri := range resourcesToDelete {
	...
	for _, unstructuredObj := range desiredResources {
		_, err = k8sClient.CreateOrUpdate(context.Background(), unstructuredObj)
		if err != nil {
			log.Error(err, "unable to create or update resource", "resource", resourceKey(unstructuredObj))
			return err
		}
	}
...
// Helper function to generate a unique key from a ResourceIdentifier.
func resourceKeyFromRI(ri ResourceIdentifier) string {
...
func deleteResource(ri ResourceIdentifier, k8sClient K8sClient, log logr.Logger) error {
	...
	err := k8sClient.Delete(context.Background(), obj.GroupVersionKind().Group, obj.GetKind(), obj.GetNamespace(), obj.GetName())
