...
func processTemplateResources(
	...
	for _, ri := range existingResourceIdentifiers {
		...
		if _, exists := desiredResources[ri]; !exists {
	...
	wm.UpdateGeneratedResources(template.Name, desiredResourceIdentifiers)
	return nil
}
...
func resourceKeyFromRI(ri ResourceIdentifier) string {
	return fmt.Sprintf("%s/%s/%s/%s", ri.Group, ri.Kind, ri.Namespace, ri.Name)
}
